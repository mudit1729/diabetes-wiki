---
title: "Doctor-Facing Algorithms"
type: concept
status: active
updated: 2026-05-17
tags:
  - algorithm
  - clinical-workflow
  - diabetes
  - navigation
clinical_review: false
---

# Doctor-Facing Algorithms

Use these pages as practical, evidence-linked workflows. They are not a substitute for clinical judgment, current labels, formulary rules, or local guidelines.

## Algorithms

- [[wiki/algorithms/type-2-diabetes-with-ckd|Type 2 diabetes with CKD]]
- [[wiki/algorithms/type-2-diabetes-with-ascvd|Type 2 diabetes with ASCVD or high ASCVD risk]]
- [[wiki/algorithms/type-2-diabetes-with-heart-failure|Type 2 diabetes with heart failure]]
- [[wiki/algorithms/insulin-initiation|Insulin initiation and intensification]]
- [[wiki/algorithms/hypoglycemia-risk|Hypoglycemia risk and de-intensification]]

## Comparison Tables

- [[wiki/taxonomies/evidence-comparison-tables|Evidence Comparison Tables]]

## Medication and Scenario Pages

- [[wiki/medications/overview|Medication Pages]]
- [[wiki/scenarios/overview|Common Clinic Scenarios]]

## General Use

1. Identify the dominant risk domain: CKD, heart failure, ASCVD, hypoglycemia, obesity, cost/access, pregnancy, or acute illness.
2. Choose drugs for outcome benefit first when CKD, heart failure, or ASCVD is present.
3. Use A1C as one input, not the only input.
4. Check contraindications, eGFR thresholds, drug interactions, affordability, and patient preference.
5. Reassess after 4-12 weeks for tolerability, hypoglycemia, weight, kidney function, and whether the treatment goal is actually being met.

## Evidence Links

- [[wiki/guidelines/ada-standards-of-care|ADA Standards of Care]]
- [[wiki/guidelines/kdigo-diabetes-ckd|KDIGO / ADA-KDIGO Diabetes in CKD]]
- [[wiki/trials/ukpds|UKPDS]]
- [[wiki/trials/accord-advance-vadt|ACCORD / ADVANCE / VADT]]
- [[wiki/trials/sglt2-cvot-program|SGLT2 CVOT Program]]
- [[wiki/trials/glp1-cvot-program|GLP-1 CVOT Program]]
- [[wiki/trials/flow|FLOW]]
